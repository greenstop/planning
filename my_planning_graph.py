...
class PlanningGraph():
    ...
    def __init__(self, problem: Problem, state: str, serial_planning=True):
        ...
        self.all_noop_actions = self.noop_actions(self.problem.state_map);
        self.all_prob_actions = self.problem.actions_list;
        self.all_actions = self.all_prob_actions + self.all_noop_actions;
        self.s_levels = []
        self.a_levels = []
        self.create_graph()
    ...
    def add_action_level(self, level):
        ...
        #   for the problem and add a PgNode_a to a_levels[0] set iff all prerequisite
        #   literals for the action hold in S0.  This can be accomplished by testing
        ...
        for s in self.s_levels[level]:
            if s.is_pos == True:
                n = PgNode_a(Action(expr("Noop_pos(%s)"%(s.symbol)),
                                    [[s.symbol],[]],
                                    [[s.symbol],[]]
                                    ));
            else:
                n = PgNode_a(Action(expr("Noop_neg(%s)"%(s.symbol)),
                                    [[],[s.symbol]],
                                    [[],[s.symbol]]
                                    ));
            actions.add(n);

        #add actions when preconditions are met
        s_map = { s.literal:s.is_pos for s in self.s_levels[level]};
        def check_precondition(action):
            for fluent in action.precond_pos:
                if fluent not in s_map: return False;
                if s_map[fluent] == False: return False;
            for fluent in action.precond_neg:
                fluent=expr("~%s"%fluent)
                if fluent not in s_map: return False;
                if s_map[fluent] == True: return False;
            return True;
        for action in self.all_prob_actions:
            if check_precondition(action) is True:
                actions.add(PgNode_a(action));

        #add parents, children
        s_map = { s.literal:s for s in self.s_levels[level]};
        for action in actions:
            for n in action.prenodes:
                if n.literal in s_map:
                    parent = s_map[n.literal];
                    if n == parent:
    ...
    def add_literal_level(self, level):
        ...
        #union all effect nodes
        for a in actions:
            state |= a.effnodes;
            
        #add parents, children
        s_map = { s.literal:s for s in state};
        for action in actions:
            for n in action.effnodes:
                if n.literal in s_map:
                    child = s_map[n.literal];
                    if n == child:
                        child.parents.add(action);
                        action.children.add(child);

        self.s_levels.append(state);
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        for effect0 in node_a1.effnodes:
            for effect1 in node_a2.effnodes:
                if effect1.symbol == effect0.symbol:
                    if effect1.is_pos ^ effect0.is_pos:
                        return True;
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for parent0 in node_a1.parents:
            for parent1 in node_a2.parents:
                if parent1.is_mutex(parent0):
                    return True;
        for precond0 in node_a1.prenodes:
            for precond1 in node_a2.prenodes:
                if precond1.symbol == precond0.symbol:
                    if precond1.is_pos ^ precond0.is_pos:
                        return True;
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        '''
        for a0 in node_s1.parents:
            for a1 in node_s2.parents:
                if a0.is_mutex(a1) == False:
                    return False;
        return True;
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            for level in self.s_levels:
                if g in { s.literal for s in level}:
                    break;
                level_sum+=1;
        return level_sum
